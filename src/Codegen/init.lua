...
${SerializedFlags}${LoadModuleCode}]],
    Minified = [[
-- This script was automatically @generated by Maui, it is not intended for manual editing.
...
-- To check if a table has the same values as another table
local function TableHasSameValues(inputTable, tableToCompareWith)
    for Key, Value in inputTable do
        if type(Value) == "table" then
            if not TableHasSameValues(Value, tableToCompareWith[Key]) then
                return false
            end
        elseif tableToCompareWith[Key] ~= Value then
            return false
        end
    end

    -- Fallback to true, we want to denote it in the codegen if so
    return true
end
...
local function BuildFromSelection(selection, log, options, projectFileModule)
    ...
    -- Serialize flags, if needed!
    local SerializedFlags = if options and options.Flags and not TableHasSameValues(options.Flags, DefaultFlags) then
            LuaEncode(options.Flags, {
                PrettyPrinting = if MinifyTable then false else true,
                IndentCount = if MinifyTable then 0 else 4,
                StackLimit = 2, -- It's only supposed to be 1 stack of values
            })
        else
            nil
    ...
    local GeneratedScript = ReplaceString(if MinifyTable then CodegenFormatTemplates.Minified else CodegenFormatTemplates.Normal, {
        ...
        SerializedFlags = if SerializedFlags then
            if MinifyTable then
                "local Flags=" .. SerializedFlags
            else
                "local Flags = " .. SerializedFlags .. "\n\n" -- The newlines seperate this tbl and the loadmodulecode, see the format templates
        else
            "",
